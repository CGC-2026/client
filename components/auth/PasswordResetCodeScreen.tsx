...
import { ActivityIndicator, Pressable, SafeAreaView, ScrollView, View, Text } from "react-native";
...
import { VerificationCodeFormData, verificationCodeSchema } from "@/app/types/authSchemas";
...
export default function PasswordResetCodeScreen({ email, next }: PasswordResetCodeScreenProps) { 
    ...
    const {
      ...
      const onResendEmail = async () => {
        ...
        try {
          ...
          setError('code', { message: err.errors?.[0]?.message || 'Failed to resend reset email. Please try again.' });
        }
      };
      ...
      const onCodeSubmit = async (data: VerificationCodeFormData) => {
        ...
        try {
          ...
          if (res.status === 'needs_new_password') {
            ...
            setError('code', { message: 'Invalid or expired code. Try again or resend.' });
          }
        } catch (err: any) {
          console.error('Code verification error:', err);
          setError('code', { message: err.errors?.[0]?.message || 'Invalid verification code. Please try again.' });
        }
      };
    return (
        ...
        <CustomKeyboardAvoidingView
          ...
          <ScrollView>
            <View style={styles.content}>
              ...
              <InfoMessage>
                Enter the 6-digit code you received in your email to continue with password reset.
              </InfoMessage>
